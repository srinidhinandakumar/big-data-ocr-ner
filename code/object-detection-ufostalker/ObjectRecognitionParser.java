...
public class ObjectRecognitionParser extends AbstractParser implements Initializable {
    ...
    public synchronized void parse(InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context)
        ...
        if (objects != null && !objects.isEmpty()) {
            ...
            List<String> acceptedLabel = new ArrayList<String>();
            ...
            for (RecognisedObject object : objects) {
                if (object instanceof CaptionObject) {
                    ...
                    acceptedLabel.add(object.getLabel());
                    flag=1;
            ...
            // ------------------- Changes done for assignment 2 ---------------------
            // Opens a file and appends the detected objects or the caption to it, makes it easier to process a bulk of images

            OutputStream outputStream;

            if(flag == 1) {
                outputStream = new FileOutputStream(new File("caption.txt"), true);
            } else {
                outputStream = new FileOutputStream(new File("objects.txt"), true);
            }

            Writer outputStreamWriter = new OutputStreamWriter(outputStream, Charset.forName("UTF-8"));

            String out_str = metadata.get("resourceName");
            out_str += "---[";
            String last_ele = acceptedLabel.get(acceptedLabel.size()-1);

            for (String x: acceptedLabel) {
                out_str += x;
            ...
            outputStreamWriter.write(out_str + "\n");
